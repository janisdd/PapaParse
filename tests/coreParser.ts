...
// import * as papaparse from '../dist/papaparse.umd'
import {Papa, Parser, ParseConfig, ParseConfigEffective, ParseResultMeta, __parseConfigUserDefaults} from '../papaparse'
// import type {ParseConfig} from '../papaparse'
// const Papa = papaparse.Papa
...
const CORE_PARSER_TESTS: TestType[] = [
  {
    description: "One row",
    input: 'A,b,c',
    expected: {
  ...
  },
  {
    ...
    input: '# Comment!\na,b,c',
    config: {comments: defaultCommentChar},
    expected: {
      data: [['a', 'b', 'c']],
      errors: []
    }
  ...
  },
  {
    ...
    expected: {
      data: [['a'], ['b'], [''], [''], ['c'], ['d'], ['e'], ['']],
      errors: [delimNotDetectedError] //inner parser does not throw errors but we want to keep the test
    }
  },
  {
  ...
  {
    description: "Fast mode with blank line at end 2",
    input: 'a,b,c\n',
...
describe('Core Parser Tests', function() {
  function generateTest(test: TestType) {
    it(test.description, function() {
      ...
      // const effectiveConfig: ParseConfigEffective = {
      //   ...__parseConfigUserDefaults,
      //   delimiter: ',',
      //   ...test.config,
      //   newline: '\n',
      //   comments: test.config?.comments ?? '',
      //   previewInRows: test.config?.previewInRows ?? 0
      // }
      // const _parser = new Parser(effectiveConfig, false)
      // const actual = _parser.parse(test.input)
